...
we can create word trees in which users can select from a list of top words to view the context in which the words appear in. A word tree in this context would prove useful for users looking to understand the different recurrent themes and phrases relating to a word. The packages explored would include that of: [`gvisWordTree`](https://search.r-project.org/CRAN/refmans/googleVis/html/gvisWordTree.html)
